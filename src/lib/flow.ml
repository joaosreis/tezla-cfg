...
let flow counter s =
  ...
  let rec flow_rec (nodes, flow) s =
    ...
    | S_map ((c, (c_1, c_2)), (r, (r_1, r_2)), b) ->
        let phi_1_id, nodes =
          new_node nodes (Cfg_assign (c, E_phi (c_1, c_2)))
        in
        let phi_2_id, nodes =
          new_node nodes (Cfg_assign (r, E_phi (r_1, r_2)))
        in
    ...
    | S_failwith e ->
        ...
        (nodes, flow)
    | S_return e ->
        let n, nodes = new_node ~id:s.id nodes (Cfg_return e) in
        let () = Hashtbl.add init_ht s.id n in
        let () = Hashtbl.add final_ht s.id n in
        (nodes, flow)
  in
  ...
  let flow =
    Set.map
      (fun (a, b) -> (Hashtbl.find nodes_ht a, Hashtbl.find nodes_ht b))
      flow
  in
